...
# Test scenario: Give 3 thumbs down on same document in different conversations
# Dashboard should show "3 Sources manquantes"
# Tab "Documents à réingérer" should list the document
```
...
```javascript
// Frontend error
POST /api/analytics/quality/chunk/{id}/unblacklist 422 (Unprocessable Content)
Error unblacklisting chunk: Request failed with status code 422
```
...
async def unblacklist_chunk(
    chunk_id: UUID,
    reason: str = Body(...),  # ✅ Explicit JSON body
    ...
)
```
...
**Files Modified**:
- `web-api/app/routes/analytics.py` (added `Body` import + 3 endpoints)

**Testing**:
```bash
# Test via UI
1. Go to Quality Management → Blacklisted Chunks
2. Click "Déblacklister" on a chunk
3. Enter reason: "Test unblacklist"
4. Should succeed without HTTP 422

# Test via API
curl -X POST http://localhost:8000/api/analytics/quality/chunk/{chunk_id}/unblacklist \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"reason": "Test via API"}'
...
- ✅ Multi-conversation scenario works correctly (3 thumbs down = 1 document shown)
...
WHERE v.sources_used IS NOT NULL
...
```bash
# View sync logs (auto-creation)
docker-compose logs -f analytics-worker | grep "Auto-synced"

# View sync logs (admin validation)
docker-compose logs -f ragfab-api | grep "marked for reingestion"

# Count validations vs quality_scores
docker-compose exec postgres psql -U raguser -d ragdb -c "
SELECT
  (SELECT COUNT(*) FROM thumbs_down_validations WHERE admin_action = 'mark_for_reingestion') as validations,
  (SELECT COUNT(*) FROM document_quality_scores WHERE needs_reingestion = true) as quality_scores;"
```

**Verify Endpoint Fixes**:
```bash
# Check FastAPI logs for 422 errors
docker-compose logs ragfab-api | grep "422"
