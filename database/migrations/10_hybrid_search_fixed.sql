-- Migration 10: Hybrid Search (BM25 + Vector) - VERSION CORRIGÉE
...
-- =====================================================
-- Automatically uses match_chunks_smart (from migration 09) with hybrid support
...
BEGIN
    -- Check if parent-child chunks exist
    IF EXISTS (SELECT 1 FROM chunks WHERE chunk_level = 'child' LIMIT 1) THEN
...
COMMENT ON FUNCTION match_chunks_smart_hybrid IS 'Smart hybrid search that automatically handles both hybrid (BM25+vector) and parent-child chunks';
