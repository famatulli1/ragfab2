"""
Provider Mistral pour PydanticAI avec support complet des function calls/tools.
Compatible avec l'API Mistral AI (https://api.mistral.ai).
"""
...
class MistralStreamTextResponse(StreamTextResponse):
    """Implementation of StreamTextResponse pour les modèles Mistral."""
...
class MistralAgentModel(AgentModel):
    """AgentModel pour Mistral avec support des tools."""
    ...
    def __init__(
        ...
        # Convert PydanticAI ToolDefinition to Mistral API format
        self.mistral_tools = self._convert_tools(function_tools)

    def _convert_tools(self, tools: list[ToolDefinition]) -> list[dict]:
        """Convert PydanticAI tools to Mistral API format."""
        mistral_tools = []
        for tool in tools:
    ...
    async def request(
        ...
        if self.mistral_tools:
            payload["tools"] = self.mistral_tools
        ...
        # Debug: log the payload (commented for cleaner output)
        # logger.debug(f"Mistral API payload: {json.dumps(payload, indent=2)}")
    ...
    async def request_stream(
        ...
        async with self.http_client.stream(
            ...
            if response.status_code != 200:
                ...
                logger.error(f"Mistral API error {response.status_code}: {error_text.decode()}")
            response.raise_for_status()
            ...
            yield MistralStreamTextResponse(
                _first_content=None,
    ...
    def _format_messages(self, messages: list[ModelMessage]) -> list[dict]:
        """Convertit les messages PydanticAI au format Mistral API."""
        formatted = []

        # Debug: log the incoming messages (commented for cleaner output)
        # logger.debug(f"Formatting {len(messages)} messages:")
        # for i, msg in enumerate(messages):
        #     logger.debug(f"  Message {i}: {type(msg).__name__} with parts: {[type(p).__name__ for p in getattr(msg, 'parts', [])]}")
...
class MistralModel(Model):
    """Modèle Mistral pour PydanticAI."""
    ...
    def __init__(
        ...
        if not self.api_key:
            raise ValueError(
                "MISTRAL_API_KEY doit être définie dans l'environnement ou passée au constructeur"
            )

    async def agent_model(
        self, *, function_tools: list[ToolDefinition], allow_text_result: bool, result_tools: list
    ) -> AgentModel:
        """Retourne une instance de MistralAgentModel."""
...
def get_mistral_model() -> MistralModel:
    """Factory function pour créer le modèle Mistral depuis les variables d'environnement."""
    return MistralModel(
        model_name=os.getenv("MISTRAL_MODEL_NAME", "open-mistral-7b"),
        api_key=os.getenv("MISTRAL_API_KEY"),
        api_url=os.getenv("MISTRAL_API_URL", "https://api.mistral.ai"),
        timeout=float(os.getenv("MISTRAL_TIMEOUT", "120.0")),
    )
